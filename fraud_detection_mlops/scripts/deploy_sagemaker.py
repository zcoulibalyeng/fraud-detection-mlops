...
Called by GitHub Actions deploy workflow.
Passes S3 artifact location as environment variables so the
container downloads the model at startup — no artifacts baked in.
"""
...
def main(
    ...
    endpoint_name: str = typer.Option(..., help="SageMaker endpoint name"),
    region: str = typer.Option("us-east-1", help="AWS region"),
    artifact_bucket: str = typer.Option("", help="S3 bucket with model artifacts"),
    artifact_prefix: str = typer.Option(
        "artifacts/latest", help="S3 prefix for artifacts"
    ),
    wait: bool = typer.Option(True, help="Wait for deployment to complete"),
) -> None:
    """Deploy a new container image to a SageMaker endpoint."""
    ...
    # Container environment — tells predictor where to find artifacts
    container_env = {}
    if artifact_bucket:
        container_env["ARTIFACT_BUCKET"] = artifact_bucket
        container_env["ARTIFACT_PREFIX"] = artifact_prefix
