...
Orchestrates the full training pipeline:
  ...
  5. Run evaluation gate vs champion
  6. Register to MLflow registry if gate passes
  7. Save artifacts to S3 for production serving
...
def upload_artifacts_to_s3(
    ...
    """Upload model and pipeline to S3 for production serving."""
    import boto3
...
def run_training_pipeline(
    ...
    with mlflow.start_run() as run:
        ...
        # Log pipeline as artifact — required for serving
        mlflow.sklearn.log_model(pipeline, name="feature_pipeline")
        logger.info("Feature pipeline logged to MLflow")
        ...
        # ── Step 6: Save model and pipeline artifacts ──────────────────
        logger.info("=== Step 6: Saving model ===")
        ...
        # Save fitted pipeline as joblib
        pipeline_path = output / "feature_pipeline.joblib"
        _joblib.dump(pipeline, pipeline_path)
        logger.info("Fitted pipeline saved to {}", pipeline_path)

        # Log artifacts to MLflow
        mlflow.log_artifact(str(model_path), "model_artifact")
        mlflow.log_artifact(str(pipeline_path), "model_artifact")

        # Log XGBoost model natively (required for register_model)
        mlflow.xgboost.log_model(
            xgb_model=model._model,
            name="model",
            input_example=X_test[:1],
        ...
        # ── Step 6b: Upload to S3 if bucket is configured ─────────────
        artifact_bucket = os.getenv("ARTIFACT_BUCKET")
        artifact_prefix = os.getenv("ARTIFACT_PREFIX", "artifacts/latest")

        if artifact_bucket:
            logger.info("=== Step 6b: Uploading artifacts to S3 ===")
            ...
            logger.info("ARTIFACT_BUCKET not set — skipping S3 upload")
...
def main(
    ...
    val_path: str = typer.Option(..., help="Path to val.parquet"),
    test_path: str = typer.Option(..., help="Path to test.parquet"),
    output_dir: str = typer.Option("models", help="Directory to save model"),
    no_register: bool = typer.Option(False, help="Skip MLflow registration (dry run)"),
) -> None:
    """Train the fraud detection model and run the evaluation gate."""
    result = run_training_pipeline(
